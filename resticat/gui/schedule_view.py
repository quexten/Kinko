...
class BackupConfigView(Gtk.Box):
    ...
    def __init__(self, backup_store, navigate_callback):
        ...
        def on_allow_on_metered_changed(switch, gparam):
            self.backup_config.schedule.allow_on_metered = switch.get_active()
        ...
        self.backup_schedule_view_backup_settings_allow_on_battery_row = Adw.SwitchRow()
        self.backup_schedule_view_backup_settings_allow_on_battery_row.set_title("Allow on battery")
        self.backup_schedule_view_backup_settings_allow_on_battery_row.set_active(True)
        self.backup_schedule_view_backup_settings_allow_on_battery_row.set_icon_name("battery-symbolic")
        def on_allow_on_battery_changed(switch, gparam):
            self.backup_config.schedule.allow_on_battery = switch.get_active()
        self.backup_schedule_view_backup_settings_allow_on_battery_row.connect("notify::active", on_allow_on_battery_changed)
        self.backup_schedule_view_backup_settings.add(self.backup_schedule_view_backup_settings_allow_on_battery_row)

        self.backup_schedule_view_backup_settings_allow_on_powersaver_row = Adw.SwitchRow()
        self.backup_schedule_view_backup_settings_allow_on_powersaver_row.set_title("Allow on powersaver")
        self.backup_schedule_view_backup_settings_allow_on_powersaver_row.set_active(True)
        self.backup_schedule_view_backup_settings_allow_on_powersaver_row.set_icon_name("battery-low-symbolic")
        def on_allow_on_powersaver_changed(switch, gparam):
            self.backup_config.schedule.allow_on_powersaver = switch.get_active()
        self.backup_schedule_view_backup_settings_allow_on_powersaver_row.connect("notify::active", on_allow_on_powersaver_changed)
        self.backup_schedule_view_backup_settings.add(self.backup_schedule_view_backup_settings_allow_on_powersaver_row)
        ...
        def on_backup_enabled_changed(switch, gparam):
            self.backup_config.schedule.backup_schedule_enabled = switch.get_active()
        ...
        def on_backup_frequency_changed(selection, gparam):
            ...
            if selected == 0:
                self.backup_config.schedule.backup_frequency = "hourly"
            elif selected == 1:
                self.backup_config.schedule.backup_frequency = "daily"
            elif selected == 2:
                self.backup_config.schedule.backup_frequency = "weekly"
            backup_schedule = self.self.backup_config.schedule
        ...
        def on_cleanup_keep_hourly_changed(switch, gparam):
            self.backup_config.schedule.cleanup_keep_hourly = switch.get_value()
        ...
        def on_cleanup_keep_daily_changed(switch, gparam):
            self.backup_config.schedule.cleanup_keep_daily = switch.get_value()
        ...
        def on_cleanup_keep_weekly_changed(switch, gparam):
            self.backup_config.schedule.cleanup_keep_weekly = switch.get_value()
        ...
        def on_cleanup_keep_monthly_changed(switch, gparam):
            self.backup_config.schedule.cleanup_keep_monthly = switch.get_value()
        ...
        def on_cleanup_keep_yearly_changed(switch, gparam):
            self.backup_config.schedule.cleanup_keep_yearly = switch.get_value()
        ...
        def on_cleanup_enabled_changed(switch, gparam):
            if "selcted_id" in self.__dict__:
                self.backup_config.schedule.cleanup_schedule_enabled = switch.get_active()
    ...
    def navigate_to(self, param, window):
        ...
        def on_back_button_clicked(button):
            # save
            self.backup_store.add_backup_config(self.backup_config)
        ...
        self.backup_config = self.backup_store.get_backup_config(self.selected_id)

        backup_schedule = self.backup_config.schedule
        ...
        self.backup_schedule_view_backup_settings_allow_on_battery_row.set_active(backup_schedule.allow_on_battery)
        self.backup_schedule_view_backup_settings_allow_on_powersaver_row.set_active(backup_schedule.allow_on_powersaver)
